...
class WordOrderCaptchaSolver(BaseCaptchaSolver):
    CAPTCHA_TYPE: str = "word_order"
    ...
    def solve(self, image_path: Optional[str] = None, image_data: Optional[bytes] = None, 
              target_order: Optional[str] = None, **kwargs) -> List[Tuple[int, int]]:
        self.validate_input(image_path=image_path, image_data=image_data, target_order=target_order)
        self.logger.info(f"Solving word order captcha: {image_path or 'image_data provided'}, order: {target_order}")
        ...
        try:
            ...
            if solver_type == "tesseract":
                positions = self._solve_with_tesseract(preprocessed_image, target_order)
            else:
                positions = self._solve_with_api(preprocessed_image, target_order)
            
            self.logger.info(f"Word order captcha solved, positions: {positions}")
            return positions
        
        except Exception as e:
            self.logger.error(f"Failed to solve word order captcha: {str(e)}")
            raise CaptchaRecognitionError(f"Failed to solve word order captcha: {str(e)}")
    ...
    def _solve_with_tesseract(self, image: Image.Image, target_order: str) -> List[Tuple[int, int]]:
        ...
        try:
            ...
            for char in target_order:
                if char in char_positions:
                    ...
                    self.logger.warning(f"Character '{char}' not found in image")
            
            if len(positions) < len(target_order):
                self.logger.warning(f"Only found {len(positions)} of {len(target_order)} target characters")
    ...
    def _solve_with_api(self, image: Image.Image, target_order: str) -> List[Tuple[int, int]]:
    ...
    def validate_input(self, image_path: Optional[str] = None, image_data: Optional[bytes] = None, 
                       target_order: Optional[str] = None, **kwargs) -> None:
        if not image_path and not image_data:
            self._raise_processing_error("Either image_path or image_data must be provided")
        if not target_order:
            self._raise_processing_error("target_order must be provided")
